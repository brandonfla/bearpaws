...
if echo "$output" | grep -qi "bearpaws:brainstorming\|bearpaws:using-bearpaws\|Available skills"; then
...
echo "  Running opencode with bearpaws:brainstorming skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load bearpaws:brainstorming and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "brainstorming\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded bearpaws:brainstorming skill"
else
    echo "  [FAIL] use_skill did not load bearpaws:brainstorming correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
